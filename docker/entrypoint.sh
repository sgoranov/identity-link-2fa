...
echo "SELECT 'CREATE DATABASE \"php-identity-link-2fa\"' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '\"php-identity-link-2fa\"')\gexec" \
 | psql -v ON_ERROR_STOP=1
php bin/console -e dev doctrine:migrations:migrate --no-interaction

echo "SELECT 'CREATE DATABASE \"php-identity-link-2fa-test\"' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '\"php-identity-link-2fa-test\"')\gexec" \
 | psql -v ON_ERROR_STOP=1
